...
static void derive_transfer_key(uint8_t *key_out) {
    /*
     * Transfer key is provisioned via secrets header.  This avoids a
     * compile-time constant in the firmware binary; different builds
     * (and devices) receive different keys.  The key is generated by
     * the host-side gen_secrets tool and placed into secrets.h as
     * TRANSFER_KEY.
     */

    extern const uint8_t TRANSFER_KEY[KEY_SIZE];
    memcpy(key_out, TRANSFER_KEY, KEY_SIZE);
}
