...
def secrets_to_c_header(
    ...
    # Parse the blob generated by gen_secrets.py; currently JSON with
    # a transfer key hex string.
    try:
        obj = json.loads(secrets.decode())
    except Exception:
        raise ValueError("Unable to parse secrets file")

    transfer_key_bytes = None
    if "transfer_key" in obj:
        hexstr = obj["transfer_key"]
        transfer_key_bytes = bytes.fromhex(hexstr)
        if len(transfer_key_bytes) != 16:
            raise ValueError("transfer_key in secrets file has wrong length")
    else:
        # fall back to all-zero key if file not populated, to keep builds
        # from completely breaking.
        transfer_key_bytes = b"\x00" * 16
    ...
    with open(os.path.join(path, "secrets.h"), 'w') as f:
        ...
        f.write('#include "crypto.h"\n')
        f.write('#include <stdint.h>\n')
        f.write("\n")
        f.write(f'#define HSM_PIN "{hsm_pin}"\n\n')

        # write transfer key as a C byte array
        f.write(f'static const uint8_t TRANSFER_KEY[{KEY_SIZE}] = {{')
        for i, b in enumerate(transfer_key_bytes):
            if i % 8 == 0:
                f.write("\n    ")
            f.write(f"0x{b:02x}, ")
        f.write("\n};\n\n")
