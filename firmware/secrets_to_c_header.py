...
def _load_transfer_key(secrets: bytes) -> bytes:
    data = json.loads(secrets.decode())
    key_hex = data.get("transfer_key_hex", "")
    key = bytes.fromhex(key_hex) if isinstance(key_hex, str) else b""
    if len(key) == 16:
        return key

    # Backwards-compatible fallback for older secrets files.
    import hashlib
    return hashlib.sha256(secrets).digest()[:16]
...
def secrets_to_c_header(
    ...
    transfer_key = _load_transfer_key(secrets)
    ...
    with open(os.path.join(path, "secrets.h"), 'w') as f:
        ...
        f.write("const static uint8_t TRANSFER_KEY[16] = {\n")
        f.write("\t" + ", ".join(f"0x{b:02x}" for b in transfer_key) + "\n")
        f.write("};\n\n")
